...
class Front3dDataset(Dataset):
    ...
    def __init__(self, cfg, class_names, batch_size, split='training', training=True, logger=None):
        ...
        if cfg.SAMPLE_RATE != 1:
            # sample_num = int(len(self.data_list) * cfg.SAMPLE_RATE)
            # self.data_list = random.choices(self.data_list, k = sample_num)
            sample_num = int(1 / cfg.SAMPLE_RATE)
            self.data_list = self.data_list[::sample_num]
    ...
    def __getitem__(self, item):
        ...
        if self.training and self.aug.enabled:
            ...
            # xyz_middle_org, label_org = data_dict['xyz_middle_org'], data_dict['label_org']
        else:
            xyz_middle = xyz.copy()
...
if __name__ == '__main__':
    ...
    with open('/cluster/personal/DODA/cfgs/da_front3d_s3dis/spconv.yaml', 'r') as f:
        try:
    ...
    train_dataset = Front3dDataset(cfg, None, None)
    xyz = train_dataset[0][0]
    print(xyz.shape)