...
def get_furniture_bounding_boxes(aug, xyz, labels, class_names):
    bounding_boxes = {}
    front_labels = np.unique(labels)
    sx_min, sx_max = xyz[labels == 1][:, 0].min(), xyz[labels == 1][:, 0].max()
    sy_min, sy_max = xyz[labels == 1][:, 1].min(), xyz[labels == 1][:, 1].max()
    bounding_boxes[class_names[1]] = (sx_min, sy_min, sx_max, sy_max)
    not_front_list = ['wall', 'floor', 'door', 'window', 'ceiling', 'beam', 'column']
    for front_label in front_labels[:-1]:
        if class_names[front_label] not in not_front_list:
            xyz_front = xyz[labels == front_label]
            x_min, x_max = xyz_front[:, 0].min(), xyz_front[:, 0].max()
            y_min, y_max = xyz_front[:, 1].min(), xyz_front[:, 1].max()
            bounding_boxes[class_names[front_label]] = (x_min, y_min, x_max, y_max)
    return bounding_boxes

def is_valid(xyz_front, bounding_boxes, class_name):  ## check colliding
    x_min, x_max = xyz_front[:, 0].min(), xyz_front[:, 0].max()
    y_min, y_max = xyz_front[:, 1].min(), xyz_front[:, 1].max()
    if x_min < bounding_boxes['floor'][0] or y_min < bounding_boxes['floor'][1] or x_max > bounding_boxes['floor'][2] or y_max > bounding_boxes['floor'][3]:
        return False
    for key in bounding_boxes.keys():
        if key not in [class_name, 'floor']:
            x_min1, y_min1, x_max1, y_max1 = bounding_boxes[key]     
            if x_min > x_max1 or y_min > y_max1 or x_max < x_min1 or y_max < y_min1:
                continue
            else:
                return False
    return True

def is_near_wall(xyz_front, floor_box):  ## check colliding
    x_min, x_max = xyz_front[:, 0].min(), xyz_front[:, 0].max()
    y_min, y_max = xyz_front[:, 1].min(), xyz_front[:, 1].max()
    wall_dist = 0.05
    x_left = True if x_min < floor_box[0]+wall_dist else False
    x_right = True if x_max > floor_box[2]-wall_dist else False
    y_down = True if y_min < floor_box[1]+wall_dist else False
    y_up = True if y_max > floor_box[3]-wall_dist else False
    
    return x_left, x_right, y_up, y_down

def front_aug(aug, xyz, labels, class_names):
    assert xyz.ndim == 2
    bounding_boxes = get_furniture_bounding_boxes(aug, xyz, labels, class_names)
    front_labels = np.unique(labels)
    not_front_list = ['wall', 'floor', 'door', 'window', 'ceiling', 'beam', 'column']
    two_freedom_list = ['chair', 'table']
    one_freedom_list = ['cabinet', 'bed', 'sofa', 'bookshelf', 'desk']
    # np.savetxt('aug_no_pc.txt', xyz)
    for front_label in front_labels[:-1]:
        class_name = class_names[front_label]
        if class_name not in not_front_list:
            xyz_front = xyz[labels == front_label]
            x_min, x_max = xyz_front[:, 0].min(), xyz_front[:, 0].max()
            y_min, y_max = xyz_front[:, 1].min(), xyz_front[:, 1].max()
            mid_point = np.array([(x_min+x_max)/2, (y_min+y_max)/2, xyz_front[:, 2].min()])
            x_range = x_max - x_min
            y_range = y_max - y_min
            xyz_front = xyz_front - mid_point
            if class_name in two_freedom_list:
                if check_key(aug.scale) and check_p(aug.scale):
                    scale_rate = (np.random.rand(1, 3)-0.5) *aug.scale.value + 1
                    if is_valid((xyz_front*scale_rate+mid_point), bounding_boxes, class_name):
                        xyz_front = xyz_front * scale_rate             
                if check_key(aug.rotation) and check_p(aug.rotation):
                    m = np.eye(3)
                    theta_x = 0
                    theta_y = 0
                    theta_z = (np.random.rand() * 2 * math.pi - math.pi) * aug.rotation.value
                    Rx = np.array \
                        ([[1, 0, 0], [0, math.cos(theta_x), -math.sin(theta_x)], [0, math.sin(theta_x), math.cos(theta_x)]])
                    Ry = np.array \
                        ([[math.cos(theta_y), 0, math.sin(theta_y)], [0, 1, 0], [-math.sin(theta_y), 0, math.cos(theta_y)]])
                    Rz = np.array \
                        ([[math.cos(theta_z), math.sin(theta_z), 0], [-math.sin(theta_z), math.cos(theta_z), 0], [0, 0, 1]])
                    m = np.matmul(m, Rx.dot(Ry).dot(Rz))
                    if is_valid((np.matmul(xyz_front, m)+mid_point), bounding_boxes, class_name):
                        xyz_front = np.matmul(xyz_front, m)
                if check_key(aug.trans) and check_p(aug.trans):
                    trans_scale = (np.random.rand(1, 2)-0.5) * aug.trans.value * [x_range, y_range]
                    if is_valid((xyz_front[:, :2]+trans_scale+mid_point[:2]), bounding_boxes, class_name):
                        xyz_front[:, :2] += trans_scale
            if class_name in one_freedom_list:
                if check_key(aug.trans) and check_p(aug.trans):
                    x_left, x_right, y_up, y_down = is_near_wall(xyz_front+mid_point, bounding_boxes['floor'])
                    trans_scale = (np.random.rand(1, 2)-0.5) * aug.trans.value * [x_range, y_range]
                    if x_left == True or x_right == True:
                        trans_scale[:, 0] = 0
                    if y_up == True or y_down == True:
                        trans_scale[:, 1] = 0
                    if is_valid((xyz_front[:, :2]+trans_scale+mid_point[:2]), bounding_boxes, class_name):
                        xyz_front[:, :2] += trans_scale
            xyz_front = xyz_front + mid_point
            x_min, x_max = xyz_front[:, 0].min(), xyz_front[:, 0].max()
            y_min, y_max = xyz_front[:, 1].min(), xyz_front[:, 1].max()
            bounding_boxes[class_name] = (x_min, y_min, x_max, y_max)
            xyz[labels == front_label] = xyz_front
    # np.savetxt('aug_pc.txt', xyz)
    return xyz
# def front_aug(aug, xyz, labels, class_names):
#     assert xyz.ndim == 2
#     front_labels = np.unique(labels)
    
#     not_front_list = ['wall', 'floor', 'door', 'window', 'ceiling', 'beam', 'column', 'cabinet', 'bookshelf', 'bed']
#     for front_label in front_labels[:-1]:
#         if class_names[front_label] not in not_front_list:
#             xyz_front = xyz[labels == front_label]
#             x_min, x_max = xyz_front[:, 0].min(), xyz_front[:, 0].max()
...
#             if check_key(aug.scale) and check_p(aug.scale):
#                 xyz_front = xyz_front * ((np.random.rand(1, 3)-0.5) * aug.scale.value + 1)      
#             if check_key(aug.rotation) and check_p(aug.rotation):
#                 m = np.eye(3)
#                 theta_x = 0
#                 theta_y = 0
...
#                 xyz_front = np.matmul(xyz_front, m)
#             if check_key(aug.trans) and check_p(aug.trans):
#                 xyz_front[:, :2] += ((np.random.rand(1, 2)-0.5) * aug.trans.value * [x_range, y_range])
#             xyz_front = xyz_front + mid_point
#             xyz[labels == front_label] = xyz_front
#     return xyz
