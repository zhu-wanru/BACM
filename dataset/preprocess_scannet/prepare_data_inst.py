...
data_root = '/cluster/public_datasets/3DCommon/scannet/scans/'
# files = sorted(glob.glob(split + '/*_vh_clean_2.ply'))
files = sorted(glob.glob(data_root + '*' + '/*_vh_clean_2.ply'))
if opt.data_split != 'test':
    files2 = sorted(glob.glob(data_root + '*' + '/*_vh_clean_2.labels.ply'))
    files3 = sorted(glob.glob(data_root + '*' + '/*_vh_clean_2.0.010000.segs.json'))
    files4 = sorted(glob.glob(data_root + '*' + '/*[0-9].aggregation.json'))
...
def f(fn, file_list):
    save_root = '/cluster/sc_download/zhuwanru/scannet/'+opt.data_split+'_group/'

    if os.path.exists(save_root+fn[-27:-15]+'.pth') or (fn[-27:-15] not in file_list):
        return None
    fn2 = fn[:-3] + 'labels.ply'
    ...
    if(fn[-27:-15] == 'scene0217_00' and instance_segids[0] == instance_segids[int(len(instance_segids) / 2)]):
    ...
    instance_labels = np.ones(sem_labels.shape[0]) * 255
    ...
    torch.save((coords, colors, sem_labels, instance_labels), save_root+fn[-27:-15]+'.pth')
    # print('Saving to ' + fn[:-15]+'_inst_nostuff.pth')

file_list = []
with open('scannetv2_'+opt.data_split+'.txt', 'r') as file:
    for line in file:
