...
class S3DISDataset(Dataset):
    ...
    def load_data(self, index):
        ...
        return xyz_all, label_all, rgb_all

    def __getitem__(self, item):
        index = item % len(self.data_list)
        xyz_all, label_all, rgb_all = self.load_data(index)
        ...
        # # )
        # xyz, label, rgb = self.filter_by_index(
        #     [xyz_all, label_all, rgb_all], subsample_idx
        # )
        xyz, label, rgb = xyz_all, label_all, rgb_all
        ...
        if self.training:
            ...
            return xyz, xyz_middle, xyz_all, xyz_middle_all, label, label_all, index, rgb
    ...
    def test_collate_fn(self, items):
        ...
        for i, item in enumerate(items):
            xyz, xyz_mid, xyz_all, xyz_mid_all, label, label_all, idx, rgb = item
        ...
        return {'locs': locs, 'voxel_locs': voxel_locs, 'p2v_map': p2v_map, 'v2p_map': v2p_map, 'locs_float': locs_float, 
                ...
                'offsets': batch_offsets, 'offsets_all': batch_offsets_all, 'spatial_shape': spatial_shape, 'rgbs': rgbs}
