...
class Sam(nn.Module):
    ...
    def __init__(
        ...
        for n, value in self.image_encoder.named_parameters():
          if "cnn_embed" not in n and "pos_embed" not in n and "Adapter" not in n and "2.attn.rel_pos" not in n \
    ...
    def forward(
        ...
        low_res_masks, low_res_masks_kl = self.mask_decoder( 
        ...
        masks_kl = F.interpolate(low_res_masks_kl, (H, W), mode="bilinear", align_corners=False)
        if mode == 'src':
          return masks, masks_kl 
        else:
          return masks.detach(), masks_kl
