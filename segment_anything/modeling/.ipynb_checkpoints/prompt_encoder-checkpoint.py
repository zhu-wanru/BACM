...
class PromptEncoder(nn.Module):
    def __init__(
        ...
        self.num_point_embeddings: int = 11  # pos/neg point + 2 box corners
    ...
    def _embed_points(
        ...
        if pad: # add a not a point point
            ...
            padding_label = 255 * torch.ones((labels.shape[0], 1), device=labels.device)
            points = torch.cat([points, padding_point], dim=1)
            labels = torch.cat([labels, padding_label], dim=1)
        point_embedding = self.pe_layer.forward_with_coords(points, self.input_image_size)
        point_embedding[labels == 255] = 0.0
        point_embedding[labels == 255] += self.not_a_point_embed.weight
