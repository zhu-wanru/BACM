...
import math
from .wtconv_layer import WTConv2d
from functools import partial

# This class and its supporting functions below lightly adapted from the ViTDet backbone available at: https://github.com/facebookresearch/detectron2/blob/main/detectron2/modeling/backbone/vit.py # noqa
class ImageEncoderViT_ft(nn.Module):
    def __init__(
        self,
        img_size: int = 256,
        patch_size: int = 8,
        ...
        self.cnn_embed = ResNetEmbed(embed_dim=embed_dim, network='resnet34') # new to sam
        ...
        #     self.pos_embed = PosConvEmbed(embed_dim=embed_dim)
            ...
            # )
            # self.pos_post_embed = PosConvEmbed0(embed_dim=embed_dim)
        ...
        # self.wt_conv = WTConv2d(3, 3)

    def forward(self, x: torch.Tensor) -> torch.Tensor:

        cnnx = self.cnn_embed(x) # b h w c
        # cnnx_wt = self.cnn_embed(self.wt_conv(x))
        x = self.patch_embed(x) # b h w c
        x = self.input_Adapter(x)
        # x = x + self.pos_post_embed(self.pos_embed)
...
#         self.MLP_Adapter = Adapter(dim, mlp_ratio=1/8, skip_connect=False)
    ...
    def forward(self, x: torch.Tensor, cnnx: torch.Tensor):
        ...
        # ------------------ new to sam----------------------
        # if self.depth in self.adapted_depth:
...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        B, H, W, _ = x.shape # B, H, W, D
        qkv0 = self.qkv(x) # B, H, W, 3*D
...
class PatchEmbed0(nn.Module):
    ...
    def __init__(
        ...
        self.proj = nn.Conv2d(
            in_chans, embed_dim, kernel_size=16, stride=(16, 16), padding=padding
        )
        # self.proj = nn.Conv2d(
        #     in_chans, embed_dim, kernel_size=kernel_size[0], stride=stride[0], padding=padding
        # )
...
class PosConvEmbed0(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.conv = nn.Sequential(
            nn.Conv2d(embed_dim, embed_dim, 2, 2),
