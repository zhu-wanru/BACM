...
class SparseConvNet(nn.Module):
    def __init__(self, cfg):
        ...
        self.linear_kl = nn.Linear(mid_channel, n_classes)  # bias(default): True
    ...
    def forward(self, input, input_map, return_mid_feat=True):
        ...
        return pc_output, pc_output_kl #, output_feats.detach()

def show_points(coords, labels, labels_gt, ax, marker_size=220):

    points_2 = coords[(labels!=labels_gt) & (labels_gt!=255)]
    linewidth = 2
    color = 'black'
    ax.scatter(points_2[:, 0], points_2[:, 1], color=np.array([152, 251, 152])/255, marker='*', s=marker_size, edgecolor=color, linewidth=linewidth)
    ax.axis('off')
...
def test_model_feat(cfg, batch_size, batch, model, sam, epoch, step, test=False):
    ...
    pc_output, pc_output_kl = model(input_, p2v_map, return_mid_feat=True)   # (N, nClass), float
    ...
    #### 选择grid里对应的prompt point和对应的label
    device = imgs.device
    B, W, H, _ = imgs.shape
    sfm = torch.nn.Softmax(dim=3)
    pc2imgs_confs, pc2imgs_preds = sfm(pc2imgs_logits.detach()).max(3) 
    pc2imgs_preds[pc2imgs_confs < 0.091] = 255
    points = select_prompt_points(pc2imgs_confs, None, step, B, W, H, device)

    batch_indices = torch.arange(B, device=coords_float.device).view(B, 1).expand(B, int(W/step)*int(H/step))
    ...
    img_output, img_output_kl = sam(imgs, points, points_label, 'src')
...
def model_fn_decorator(cfg, batch_size, test=False):
    ...
    n_classes = cfg.COMMON_CLASSES.n_classes
    if cfg.OPTIMIZATION_PC.get('loss', 'cross_entropy') == 'cross_entropy':
    ...
    elif cfg.OPTIMIZATION_PC.get('loss', 'cross_entropy') == 'lovasz':
        semantic_criterion = lovasz_softmax_with_logit(ignore=ignore_label)
    else:
        raise NotImplementedError

    def test_model_fn(batch, model, sam, epoch, step, thres=0.0, with_crop=False):
        # prepare input
        ...
        img_output, img_output_kl, pc_output, pc_output_kl, pc_preds, img_labels, pc2imgs_logits, keep_idx, xs, ys, acc = test_model_feat(cfg, batch_size, batch, model, sam, epoch, step, test=True)
        device = img_output.device
        img_preds = img_output.max(1)[1] 
        img_labels = img_labels.squeeze(-1) #(B, W, H)
        weight = torch.zeros(pc_preds.size()).cuda()    # (N), float
        ...
        for i in range(batch_size):
            ...
            for v in range(V):
                img2pc_pred[keep_idx[i][v].long(), v, :] = img_output[i*V+v, :, xs[i][v].long(), ys[i][v].long()].permute(1, 0) # xs (B, N) pred_points (N, 11)
        ...
        all_output =  (img2pc_logits + pc_output) / 2
        all_preds = all_output.max(1)[1] 

        # if threshold
        pseudo_labels = pc_preds.clone()
        thres = thres if isinstance(thres, list) else [thres] * n_classes
        thres = torch.from_numpy(np.array(thres)).cuda()
        softmax_output = torch.nn.functional.softmax(pc_output, dim=1)
        ...
        if with_crop and batch['offsets'][-1] < batch['offsets_all'][-1]:
            ...
            # broadcast labels
            all_output = all_output[point_idx]
            all_preds = all_preds[point_idx]
            weight = weight[point_idx]
            # compute loss
            labels = batch['labels_all'].cuda(non_blocking=True)                      # (N), long, cuda
            loss = semantic_criterion(pc_output, labels)
        else:
            # compute loss
            labels = batch['labels'].cuda(non_blocking=True)                      # (N), long, cuda
            loss = semantic_criterion(pc_output, labels)
        ret = {'loss': loss, 'all_output': all_output, 'all_preds': all_preds, 'img2pc_preds': img2pc_preds, 
                'pc_labels': labels, 'pc_preds': pc_preds, 'pc_output': pc_output, 'acc': acc, 'weight': weight, 'pseudo_labels': pseudo_labels}
        return ret

   
    def model_fn(batch, model, sam, epoch, step, soft_label=False, loss_weight=None):
        # prepare input
        # batch {'locs': locs, 'voxel_locs': voxel_locs, 'p2v_map': p2v_map, 'v2p_map': v2p_map,
        # 'locs_float': locs_float, 'feats': feats, 'labels': labels,
        # 'offsets': batch_offsets, 'spatial_shape': spatial_shape, 'id': id}
        ...
        batch_size = batch['offsets'].size(0) - 1
        img_output, img_output_kl, pc_output, pc_output_kl, pc_preds, img_labels, pc2imgs_logits, keep_idx, xs, ys, acc = test_model_feat(cfg, batch_size, batch, model, sam, epoch, step)
        labels = batch['labels'].squeeze(-1).long().cuda(non_blocking=True)
        ...
        for i in range(batch_size):
            ...
            for v in range(V):
                img2pc_pred[keep_idx[i][v].long(), v, :] = img_output[i*V+v, :, xs[i][v].long(), ys[i][v].long()].permute(1, 0) # xs (B, N) pred_points (N, 11)
                img2pc_pred_kl[keep_idx[i][v].long(), v, :] = img_output_kl[i*V+v, :, xs[i][v].long(), ys[i][v].long()].permute(1, 0)
            img2pc_logits.append(img2pc_pred.sum(dim=1))
        ...
        all_output = img2pc_logits + pc_output
        all_preds = all_output.max(1)[1] 

        # compute loss
        T_img = 0.25
        loss_img = semantic_criterion(img_output, img_labels.long())
        ...
        loss_align_img = criterion_aign_img(F.log_softmax(img_output_kl.permute(0, 2, 3, 1) / T_img, dim=3), F.log_softmax((pc2imgs_logits.detach() / T_img), dim=3))
        loss_align_img = (loss_align_img * (pc2imgs_logits != 0)).sum() / len(torch.where(pc2imgs_logits != 0)[0])

        loss_all = loss_img + loss_pc + 0.25 * loss_img2pc + loss_align_pc + loss_align_img + 0.25 * loss_align_img2pc

        ret = {'loss': loss_all, 'output': all_output, 'all_preds': all_preds, 'img_labels': img_labels, 'img_preds': img_preds, 'img2pc_preds': img2pc_preds, 'pc_labels': labels, 'pc_output': pc_output, 'pc_preds': pc_preds, 'acc': acc}
        return ret
