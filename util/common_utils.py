...
def adjust_lr(optim_cfg, optimizer, total_epochs, total_iters_per_epoch,  epoch, iter):
...
def build_optimizer(optim_cfg, model):
    ...
    if optim_cfg.get('optim', 'sgd') == 'sgd':
        ...
        optimizer = torch.optim.Adam(params, lr=optim_cfg.base_lr, weight_decay=optim_cfg.weight_decay)
    elif optim_cfg.optim == 'adamw':
        optimizer = torch.optim.AdamW(params, lr=optim_cfg.base_lr, weight_decay=optim_cfg.weight_decay)
    else:
        raise NotImplementedError
...
if __name__ == '__main__':
    ...
    step_epoch = 70
    clip = 1e-6
    for epoch in range(-1, step_epoch):
        lr = max(base_lr * (multiplier ** (epoch // step_epoch)), clip)
        print(epoch, epoch // step_epoch, lr)
