...
def forward_test_model(batch, model, sam, model_fn, epoch, step, thres=0.0, with_crop=False):
    with torch.no_grad():
        ret = model_fn(batch, model, sam, epoch, step, thres=thres, with_crop=with_crop)
    return ret


def generate_pseudo_label_batch(
    cfg, batch, train_loader, model, sam, model_fn_st, epoch, step, pseudo_labels_dir, thres=0.7
):
    ...
    ret = forward_test_model(
        batch, model, sam, model_fn_st, epoch, step, thres=thres,
    ...
    # labels = ret['labels']
    # pseudo_acc = len(torch.where(labels[pseudo_labels != 255] == pseudo_labels[pseudo_labels != 255])) / len(torch.where(pseudo_labels[pseudo_labels != 255]))
...
def generate_pseudo_labels(
    cfg, logger, train_loader, model, sam, model_fn_st, epoch, step, pseudo_labels_dir, rank=0, thres=0.7, done=True, dist=False
):
    logger.info("******************* Generating Pseudo Labels *********************")
    common_utils.synchronize()
    if cfg.DATA_CONFIG_TAR.DATA_PROCESSOR.get('no_downsample_infer', False):
        ds = train_loader.dataset.get_downsampling_scale()  # 1
    ...
    for batch in train_loader:
        class_ratio_batch = generate_pseudo_label_batch(
            cfg, batch, train_loader, model, sam, model_fn_st, epoch, step, pseudo_labels_dir, thres=thres)
        if dist:
...
def get_label_confidence(cfg, logger, train_loader, model, sam, test_model_fn_st, epoch, step, rank, dist=False):
    ...
    for i, batch in enumerate(train_loader):
        with torch.no_grad():
            ret = forward_test_model(batch, model, sam, test_model_fn_st, epoch, step)
        # output = ret['all_output']
        # pseudo_labels = ret['all_preds']
        output = ret['pc_output']
        pseudo_labels = ret['pc_preds']
...
def get_thres_per_class_on_thres_ratio(cfg, logger, train_loader, model, sam, test_model_fn_st, epoch, step, rank, dist=False):
    """given per class thres ratio, get per class thres"""
    class_confidence, _ = get_label_confidence(
        cfg, logger, train_loader, model, sam, test_model_fn_st, epoch, step, rank, dist=dist
    )
    ...
    for c in range(len(class_confidence)):
        ...
        try:
            per_class_thres_list.append(
                min(class_confidence[c][:int(max(1, int(thres_ratio[c] * len(class_confidence[int(c)]))))][-1], cfg.SELF_TRAIN.max_thres))
        except IndexError:  # no point is predicted as this class
            per_class_thres_list.append(0.0)
    return per_class_thres_list


def get_perclass_thres(cfg, logger, train_loader, model, sam, model_fn, epoch, step, rank, dist=False):
    if cfg.SELF_TRAIN.get('global_thres', False):  # global threshold
        thres = cfg.SELF_TRAIN.thres # [0.7]
        if len(thres) == 1:
            thres = thres * cfg.COMMON_CLASSES.n_classes
    else:  # based on per class threshold
        thres = get_thres_per_class_on_thres_ratio(cfg, logger, train_loader, model, sam, model_fn, epoch, step, rank, dist=dist)
    return thres


def set_pseudo_labels(
    args, cfg, logger, pseudo_labels_dir, train_loader, model, sam, model_fn, epoch, step, rank=0, dist=False
):
    # generate pseudo labels
    generate = False
    if not os.path.exists(pseudo_labels_dir / 'done.txt'):
        thres = get_perclass_thres(cfg, logger, train_loader, model, sam, model_fn, epoch, step, rank, dist=dist)
        logger.info('per class thres: {} '.format(thres))
        class_ratio = generate_pseudo_labels(
            cfg, logger, train_loader, model, sam, model_fn, epoch, step, pseudo_labels_dir, rank, thres,
            done=True, dist=dist
        )
