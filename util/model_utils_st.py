...
def build_model(cfg):
    if cfg.MODEL.NAME == 'SparseConvNet':
        from model.unet_st import SparseConvNet as Model
        from model.unet_st import model_fn_decorator
    else:
...
def load_params_from_ckpt(ckpt, dist, model, optimizer=None, scheduler=None, logger=logging.getLogger()):
...
def load_params_from_pretrain(
    ckpt, dist, model, logger=logging.getLogger(), strict=False
):
...
def save_params(filename, model, optimizer, scheduler, epoch_log, metric=None):
    ...
    if scheduler is not None:
        torch.save(
            {'epoch': epoch_log, 'state_dict': model_state, 'optimizer': optimizer.state_dict(), 'scheduler': scheduler.state_dict(),
            'commit_id': get_git_commit_id(), 'metric': metric}, filename)
    else:
        torch.save(
            {'epoch': epoch_log, 'state_dict': model_state, 'optimizer': optimizer.state_dict(), 'scheduler': None,
            'commit_id': get_git_commit_id(), 'metric': metric}, filename)
