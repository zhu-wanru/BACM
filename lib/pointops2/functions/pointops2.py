...
try:
    ...
    pointops_cuda = load('pointops_cuda', [
        ...
        pointops_cuda_src + '/sampling/sampling_dim_cuda_kernel.cu',
...
class FurthestSampling(Function):
    ...
    def forward(ctx, xyz, offset, new_offset):
        """
        input: xyz: (n, 3), offset: (b + 1), new_offset: (b)
        output: idx: (m)
        """
        assert xyz.is_contiguous()
        n, b, n_max = xyz.shape[0], offset.shape[0] - 1, offset[0]
        for i in range(1, b):
            n_max = max(offset[i] - offset[i-1], n_max)
        idx = torch.cuda.IntTensor(new_offset[-1].item()).zero_()
        tmp = torch.cuda.FloatTensor(n).fill_(1e10)
        # pointops_cuda.furthestsampling_cuda(b, n_max, xyz, offset[1:], new_offset[1:], tmp, idx)
        pointops_cuda.furthestsampling_dim_cuda(b, n_max, xyz.shape[-1], xyz, offset[1:], new_offset[1:], tmp, idx)
        del tmp
        return idx
...
class KNNQuery(Function):
    ...
    def forward(ctx, nsample, xyz, new_xyz, offset, new_offset):
        """
        input: xyz: (n, 3), new_xyz: (m, 3), offset: (b+1), new_offset: (b+1)
        ...
        pointops_cuda.knnquery_cuda(m, nsample, xyz, new_xyz, offset[1:], new_offset[1:], idx, dist2)
...
def queryandgroup(nsample, xyz, new_xyz, feat, idx, offset, new_offset, use_xyz=True):
    ...
    grouped_xyz_diff = grouped_xyz - new_xyz.unsqueeze(1)  # coord_diff: (m, nsample, 3)
    grouped_feat = feat[idx.view(-1).long(), :].view(m, nsample, c)  # (m, nsample, c)
    # grouped_feat = grouping(feat, idx)  # (m, nsample, c)

    if use_xyz:
        return torch.cat((grouped_xyz_diff, grouped_feat), -1)  # (m, nsample, 3+c)
    else:
        return grouped_feat
